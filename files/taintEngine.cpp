...
namespace triton {
  namespace engines {
    namespace taint {
      ...
      TaintEngine::TaintEngine(const triton::modes::SharedModes& modes, triton::engines::symbolic::SymbolicEngine* symbolicEngine, triton::arch::CpuInterface& cpu, triton::arch::Architecture* architecture)
        : modes(modes),
        ...
        if (this->architecture == nullptr)
          throw triton::exceptions::TaintEngine("TaintEngine::TaintEngine(): arch cannot be null.");
      }
      ...
      bool TaintEngine::isMemoryTainted(const triton::arch::MemoryAccess& mem, bool mode) const {
        ...
        for (triton::uint32 index = 0; index < size; index++) {
          if (this->taintedMemory.find(addr+index) != this->taintedMemory.end()) {
            if (!tainted)
              std::cout << "**Tainted address: 0x" << std::hex << addr + index << " ";
            ...
            std::cout << "0x" << std::setfill('0') << std::setw(2) << unsigned(this->architecture->getConcreteMemoryValue(addr + index)) << " ";
          }
        }
        if (tainted)
          std::cout << std::endl;
        ...
        return tainted ? TAINTED : !TAINTED;
      }

      /* Returns true of false if the memory address is currently tainted */
      bool TaintEngine::isMemoryTainted2(const triton::arch::MemoryAccess& mem, bool mode) const {
        triton::uint64 addr = mem.getAddress();
        triton::uint32 size = mem.getSize();
      ...
      /* Returns true of false if the address is currently tainted */
      bool TaintEngine::isMemoryTainted2(triton::uint64 addr, triton::uint32 size) const {
        for (triton::uint32 index = 0; index < size; index++) {
          if (this->taintedMemory.find(addr+index) != this->taintedMemory.end())
            return TAINTED;
        }
      ...
      bool TaintEngine::isRegisterTainted(const triton::arch::Register& reg) const {
        if (this->taintedRegisters.find(reg.getParent()) != this->taintedRegisters.end()) {
          std::cout << "**Tainted reg: " << std::hex << reg.getName() << std::endl;
          return TAINTED;
        }
      ...
      // Original isRegisterTainted
      /* Returns true of false if the register is currently tainted */
      bool TaintEngine::isRegisterTainted2(const triton::arch::Register& reg) const {
        if (this->taintedRegisters.find(reg.getParent()) != this->taintedRegisters.end())
          return TAINTED;
