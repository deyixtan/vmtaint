...
namespace triton {
  ...
  void API::initEngines(void) {
    ...
    this->taint = new(std::nothrow) triton::engines::taint::TaintEngine(this->modes, this->symbolic, *this->getCpuInstance(), &this->arch);
  ...
  bool API::isMemoryTainted2(triton::uint64 addr, uint32 size) const {
    this->checkTaint();
    return this->taint->isMemoryTainted2(addr, size);
  }
  ...
  bool API::isMemoryTainted2(const triton::arch::MemoryAccess& mem) const {
    this->checkTaint();
    return this->taint->isMemoryTainted2(mem);
  }
  ...
  bool API::isRegisterTainted2(const triton::arch::Register& reg) const {
    this->checkTaint();
    return this->taint->isRegisterTainted2(reg);
  }
